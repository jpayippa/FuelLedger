...
4. **OCR**: Tesseract via `pytesseract.image_to_data` (word-level confidence/bounding boxes, reassembled into line text by `(block_num, par_num, line_num)`). A bounded fallback cascade (`ocr.CASCADE_ATTEMPTS`, capped at 3 attempts) tries other preprocessing-variant/PSM combinations only if the previous attempt didn't yield an amount and a date; if none validate, the first attempt's text is returned rather than the "best" one, for predictable output.
